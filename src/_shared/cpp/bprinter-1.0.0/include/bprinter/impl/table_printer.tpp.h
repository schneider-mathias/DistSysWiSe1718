...
#else
template<typename T> void TablePrinter::OutputDecimalNumber(T input, int definedPrecision){
  // If we cannot handle this number, indicate so
  int precision = definedPrecision;
  if (input < 10*(column_widths_.at(j_)-1) || input > 10*column_widths_.at(j_)){
    std::stringstream string_out;
	
	if(precision < 0)
		precision = column_widths_.at(j_);
    ...
    *out_stream_ << string_to_print;
	...
	if (precision < 0)
	{
		precision = column_widths_.at(j_) - 1; // leave room for the decimal point
		if (input < 0)
