...
class TablePrinter{
public:
  TablePrinter(std::ostream * output, const std::string & separator = "|", int decPrecision = -1);
  ~TablePrinter();
...
private:
  ...
  template<typename T> void OutputDecimalNumber(T input, int definedPreceision=-1);
