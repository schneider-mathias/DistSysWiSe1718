...
namespace bprinter {
TablePrinter::TablePrinter(std::ostream * output, const std::string & separator, int decPrecision){
  out_stream_ = output;
  i_ = 0;
  j_ = 0;
...
TablePrinter& TablePrinter::operator<<(float input){
  OutputDecimalNumber<float>(input, decimalPrecision);
  return *this;
}

TablePrinter& TablePrinter::operator<<(double input){
  OutputDecimalNumber<double>(input, decimalPrecision);
  return *this;
}
